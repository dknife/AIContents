...
원고 폴더를 못 찾으면 환경변수로 알려 주면 된다.
    ...
    AICONTENTS_WEBBOOK=/경로/AI활용_Contents/WebBook python editableserver.py
"""
import os
import sys
...
def find_webbook():
    ...
    env = os.environ.get("AICONTENTS_WEBBOOK")
    if env:
        p = Path(env).expanduser()
        if (p / "editserver.py").exists():
            return p
        sys.exit("AICONTENTS_WEBBOOK이 가리키는 곳에 editserver.py가 없습니다: %s" % p)

    cands = []
    # 원고 폴더가 이 저장소 옆에 있는 경우
    cands += [REPO.parent / "AI활용_Contents" / "WebBook",
              REPO / ".." / "WebBook"]
    # 원드라이브에 있는 경우 (계정 폴더 이름이 기기마다 달라 글롭으로 찾는다)
    home = Path.home()
    cands += sorted(home.glob(
        "Library/CloudStorage/OneDrive*/*/YMKang_Work/저술_원고/"
        "AI활용_Contents/WebBook"))
    cands += sorted(home.glob("*/YMKang_Work/저술_원고/AI활용_Contents/WebBook"))
    ...
    sys.exit(
        ...
        "환경변수로 알려 주세요:\n\n"
        "  AICONTENTS_WEBBOOK=/경로/AI활용_Contents/WebBook "
...
def main():
    ...
    os.environ["AICONTENTS_REPO"] = str(REPO)
